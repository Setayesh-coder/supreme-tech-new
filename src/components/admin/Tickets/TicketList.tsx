...
export default function TicketList() {
  ...
  const getStatusLabel = (status: string) => {
    const labels: Record<string, { label: string; icon: React.ReactElement; color: string }> = {
      OPEN: { label: "باز", icon: <AlertCircle size={14} />, color: "text-blue-400" },
      PENDING: { label: "در انتظار", icon: <Clock size={14} />, color: "text-yellow-400" },
      ANSWERED: { label: "پاسخ داده شده", icon: <CheckCircle size={14} />, color: "text-green-400" },
      CLOSED: { label: "بسته شده", icon: <XCircle size={14} />, color: "text-gray-400" },
    };
      ...
      <div className="p-4 md:p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <Link to="/admin/tickets/create-group">
              <GlassButton variant="secondary" size="sm" icon={<Users className="w-4 h-4" />} iconPosition="left">
                گروهی
              </GlassButton>
            </Link>
            <Link to="/admin/tickets/create">
              <GlassButton variant="primary" size="sm" icon={<Plus className="w-4 h-4" />} iconPosition="left">
                تیکت جدید
              </GlassButton>
            </Link>
          </div>
        ...
        {tickets.length === 0 ? (
          <LiquidGlassCard className="p-12 text-center" borderRadius="16px" blurIntensity="sm">
            ...
            <h3 className="text-xl font-bold text-white mb-2">هیچ تیکتی وجود ندارد</h3>
          ...
          <div className="space-y-3">
            {tickets.map((ticket) => {
              ...
              return (
                <LiquidGlassCard key={ticket.id} className="p-4 hover:bg-white/5 transition-all" borderRadius="16px" blurIntensity="sm" glowIntensity="sm">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="text-base font-bold text-white truncate">{ticket.title}</h3>
                        <span className={`text-xs px-2 py-0.5 rounded-full ${priority.color} bg-white/5`}>
                          {priority.label}
                        </span>
                      ...
                      <div className="flex flex-wrap gap-3 mt-1 text-xs text-gray-500">
                        ...
                        {ticket.department && <span>دپارتمان: {ticket.department}</span>}
      ...
      {showDetail && selectedTicket && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <LiquidGlassCard className="p-6" borderRadius="24px" blurIntensity="lg" glowIntensity="md">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h2 className="text-2xl font-bold text-white">{selectedTicket.title}</h2>
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-sm text-white/40">#{selectedTicket.id.slice(0, 8)}</span>
                    <span className="text-sm text-white/40">•</span>
                    <span className="text-sm text-white/40">{formatDate(selectedTicket.created_at)}</span>
                  </div>
                </div>
                <button onClick={() => setShowDetail(false)} className="p-1 hover:bg-white/10 rounded-lg">
                  <X size={24} className="text-white/60" />
                </button>
              </div>

              <div className="text-xs text-gray-500 flex gap-4 mb-4 pb-4 border-b border-white/10">
                <span className={`px-2 py-1 rounded-full ${getStatusLabel(selectedTicket.status).color} bg-white/5`}>
                  {getStatusLabel(selectedTicket.status).label}
                </span>
                <span className={`px-2 py-1 rounded-full ${getPriorityLabel(selectedTicket.priority).color} bg-white/5`}>
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto mb-4 bg-white/5 rounded-xl p-4">
                {!selectedTicket.messages || selectedTicket.messages.length === 0 ? (
                  <p className="text-center text-white/40 text-sm py-4">هنوز پیامی ارسال نشده است</p>
                ) : (
